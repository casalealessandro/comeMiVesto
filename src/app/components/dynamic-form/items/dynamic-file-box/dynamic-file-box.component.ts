...
export class DynamicFileBoxComponent implements AfterViewInit {
  ...
  async captureImage() {
    ...
    if (image &&  image.dataUrl) {  // Aggiungi un controllo per verificare che image non sia undefined
      
      
      const resizedImage = await this.resizeImage(image.dataUrl, this.maxWidth,image.format)
